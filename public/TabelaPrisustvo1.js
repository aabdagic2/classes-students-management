   ...
   for (let i = 0; i < podaci.studenti.length; i++) {
    ...
    if(podaci.prisustva.filter(x=>x.index==indeksStudenta&&x.sedmica==trenutnaSedmica).length!=0){
          ...
          kolonaPrisustvaPred.addEventListener("click", function() {
            ...
            if(this.style.backgroundColor=="lightcoral"){
               // kliknuto(predmet,indeks,trenutnaSedmica,predavanja,crveno);
                PoziviAjax.postPrisustvo(predmet,indeks,{sedmica: trenutnaSedmica, predavanja: p1+1,vjezbe: v1},tabela);
            }
            else{
                PoziviAjax.postPrisustvo(predmet,indeks,{sedmica: trenutnaSedmica, predavanja: p1-1,vjezbe: v1},tabela);
            }

           // kliknuto(predmet,indeks,trenutnaSedmica,)
          });
          if(p>0){
        ...
        for(let l=0;l<podaci.brojPredavanjaSedmicno;l++){
            ...
            kolonaPredav.addEventListener("click", function() {
                var predmet=podaci.predmet;
                var indeks=podaci.studenti[i].index;
                var pr=1;
                var vj=0;
